...
export class SeedService implements OnModuleInit {
  ...
  public async seed() {
    ...
    // Check for these specific org names; create missing orgs; for each org ensure required users exist - Idempotent seeding
    ...
    // Log existing orgs
    this.logger.log(
      `Found ${existingOrgs.length} existing org(s): ${
        [...existingNames].join(", ") || "(none)"
      }`
    );

    // Seed each org and its users
    for (const orgSeed of orgsToSeed) {
      ...
      if (!org) {
        ...
        this.logger.log(`Created org "${org.name}" with id=${org.id}`);
      } else {
        this.logger.log(
