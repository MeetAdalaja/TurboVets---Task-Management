  ...
  template: `
    <div class="space-y-4 md:space-y-6">
      ...
      <ng-container *ngIf="currentOrgId">
            ...
            >
              <div>
                ...
                <p class="mt-0.5 text-[11px] text-slate-400">
                  Drag tasks between columns to update their status. Use the
          ...
          <section class="card-elevated px-4 py-4 sm:px-5 sm:py-5">
            <header class="mb-3 flex items-center justify-between gap-2">
              ...
              <div class="flex flex-1 flex-wrap items-center justify-end gap-2">
                ...
                <div
                  *ngIf="tasks.length"
                  class="hidden items-center gap-1 rounded-full bg-slate-900/80 px-3 py-1 text-[11px] text-slate-300 sm:flex"
                >
                  <span class="h-1.5 w-1.5 rounded-full bg-emerald-400"></span>
                  <span
                    >{{ getTotalVisibleTasks() }} /
                    {{ tasks.length }} visible</span
                  >
                </div>

                <!-- Filters -->
                <div
                  class="flex flex-wrap items-center justify-end gap-2 text-[11px] text-slate-300"
                >
                  <!-- Search -->
                  <div
                  ...
                  >
                    ...
                    <option value="due_asc">Soonest due</option>
                  ...
                  >
                    ...
                    <option value="assigned">Any assignee</option>
                    <option
                      *ngFor="let m of assignableMembers"
                      [value]="m.userId"
                    >
              ...
              >
                ...
                >
                  ...
                  <span class="text-[11px] text-slate-400">
                    {{ getVisibleTasksByStatus(column.key).length }}
                  </span>
                </div>
                ...
                >
                  <div
                    *ngFor="
                      let task of getVisibleTasksByStatus(column.key);
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                ...
                                <!-- Options list -->
                                <div class="max-h-44 border-t border-slate-800">
                  ...
                  <div
                    *ngIf="!getVisibleTasksByStatus(column.key).length"
        ...
        <div
          *ngIf="pendingDeleteTask as taskToDelete"
        ...
        >
          ...
          >
            ...
            <p class="mt-2 text-[11px] text-slate-500">
              The task will be permanently removed from this board.
            </p>
...
export class TasksPageComponent implements OnInit {
  ...
  // filtering & sorting
  taskQuery = "";
  taskSortMode: "created_desc" | "created_asc" | "due_asc" | "due_desc" =
    "created_desc";
  taskAssigneeFilter: "all" | "unassigned" | "assigned" | string = "all";
  ...
  // UI gating is for UX, not security.
  ...
  ngOnInit(): void {
    // guard should prevent this route anyway
    ...
    if (!this.currentOrgId) {
      this.auth.loadOrganizations().subscribe({
        ...
        error: () => {
          ...
          this.toast.show("Failed to load organizations.", {
            type: "error",
            title: "Error",
          });
        },
  ...
  getVisibleTasksByStatus(status: TaskStatus): Task[] {
    let result = this.getTasksByStatus(status);
    ...
    // assignee filter
    switch (this.taskAssigneeFilter) {
      case "unassigned":
      ...
      default:
        if (this.taskAssigneeFilter !== "all" && this.taskAssigneeFilter) {
          const targetId = this.taskAssigneeFilter;
          console.log("Filtering by assignee ID:", targetId);
          result = result.filter((t) => t.assignedTo?.id === targetId);
        }
        break;
    }

    // sort – work on a copy so we don't mutate this.tasks
    const mode = this.taskSortMode;
    // shallow copy of an array without modifying the original result array
    return [...result].sort((a, b) => {
      const createdA = a.createdAt ? new Date(a.createdAt as any).getTime() : 0;
      const createdB = b.createdAt ? new Date(b.createdAt as any).getTime() : 0;
  ...
  getTotalVisibleTasks(): number {
    return this.columns.reduce(
      (sum, col) => sum + this.getVisibleTasksByStatus(col.key).length,
      0
    );
  }
  ...
  onDrop(event: CdkDragDrop<Task[]>, newStatus: TaskStatus): void {
    ...
    this.tasksService.updateTask(task.id, { status: newStatus }).subscribe({
      ...
      error: () => {
        ...
        this.toast.show("Failed to update task status.", {
          type: "error",
          title: "Error",
        });
      },
  ...
  loadTasks(): void {
    ...
    this.tasksService.getTasks().subscribe({
      ...
      error: () => {
        ...
        this.loading = false;
        this.toast.show("Failed to load tasks.", {
          type: "error",
          title: "Error",
        });
      },
  ...
  loadMembers(): void {
    ...
    this.orgUsersService.getOrgUsers().subscribe({
      ...
      error: () => {
        ...
        this.toast.show("Could not load organization members.", {
          type: "warning",
          title: "Members",
        });
      },
  ...
  onSubmitTask(): void {
    ...
    const payload: any = {
      ...
      dueDate: dueDateIso,
      assignedToUserId: this.newAssigneeId || null,
    };
    ...
    if (!this.editingTaskId) {
      ...
      this.tasksService.createTask(payload).subscribe({
        ...
        error: () => {
          ...
          this.toast.show("Failed to create task.", {
            type: "error",
            title: "Error",
          });
        },
  ...
  onAssigneeChange(task: Task, userId: string): void {
    ...
    this.tasksService.updateTask(task.id, { assignedToUserId }).subscribe({
      next: (updated) => {
        ...
        const label = updated.assignedTo?.fullName || "Unassigned";
        this.toast.show(`Assignee updated to ${label}.`, {
          type: "success",
          title: "Assignee",
        });
      },
      error: () => {
        ...
        this.toast.show("Failed to update assignee.", {
          type: "error",
          title: "Error",
        });
      },
  ...
  // open nice modal instead of browser confirm
  deleteTask(task: Task): void {
    this.pendingDeleteTask = task;
    this.error = "";
  }

  // user clicked "Delete task" in modal
